...
using AuthSystem.Models; // Przestrzeń nazw dla RegisterModel i LoginModel
...
public class AuthController : ControllerBase
{
    ...
    public async Task<IActionResult> Login([FromBody] LoginModel model)
    {
        ...
        if (result.Succeeded)
        {
            ...
            {
                var token = GenerateJwtToken(user);
                await SignInUserAsync(user); // Ustanów sesję opartą na ciasteczkach
                return Ok(new { Token = token, RedirectUrl = "/" }); // Zwróć token i informację o przekierowaniu
            }
            return StatusCode(500, "Wystąpił błąd podczas generowania tokenu.");
        }
        if (result.RequiresTwoFactor)
        {
            return StatusCode(403, new { Message = "Wymagane uwierzytelnianie dwuskładnikowe." });
        }
        if (result.IsLockedOut)
        {
            return StatusCode(403, new { Message = "Konto użytkownika zostało zablokowane." });
        }
        else
        {
            return Unauthorized(new { Message = "Nieprawidłowa próba logowania." });
        }
    }
